...
[**HTTP Client**](./client/overview.md) — A full-featured async HTTP client with connection pooling, TLS, and HTTP/1.0 / HTTP/1.1 / HTTP/2 / HTTP/3 support. Composable [client middleware](./client/middleware.md) (logging, cookies, compression, redirects, caching), WebSocket and WebTransport upgrades, and optional JSON helpers. Uses the same conn-based interface as the server.
