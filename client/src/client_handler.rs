...
//!    request, short-circuit by [calling `Conn::halt`] + populating synthetic response state
//!    (cache hit, mocked response), or fail. If any handler halts, subsequent `run` methods are
//!    skipped.
