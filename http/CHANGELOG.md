...
- HTTP/2 server: a bidirectional or upgraded stream — a gRPC bidi- or client-streaming RPC, a WebSocket-over-HTTP/2 connection — was torn down the moment the client finished sending its request (half-closed its side) while the server was still streaming the response. The rest of the response and any trailers were dropped, surfacing to the client as a stream reset or broken-pipe error. The server now keeps its response half open until it completes, per RFC 9113 §5.1 half-closed (remote). Relatedly, when a client reset such a stream mid-flight (cancelling an in-flight RPC), the handler's further writes were silently discarded; they now report a broken-pipe error so the handler can stop.
...
- HTTP/2 client: a request awaiting its response could hang forever when the connection closed without a response — a server graceful `GOAWAY`, a peer FIN, or an I/O error — provided the server hadn't reset that specific stream. Connection teardown woke connection-level waiters (pending PINGs, SETTINGS waiters) but left in-flight streams' recv-side waiters parked, so a task awaiting response headers, reading a response body, or writing to an upgraded stream never resumed. Teardown now resolves those streams as reset, so the waiters surface a connection-aborted / broken-pipe error instead of hanging. (A server stream reset already removed the stream and was unaffected.)
