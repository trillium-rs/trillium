...
/// Regression: a response body larger than one frame must continue emitting frames after
/// the first batch even when no peer-initiated event would wake the driver. Earlier,
/// `park()` only consulted handler signals, so the driver parked with body cursor + budget
/// still available and the response stalled after ~49KB (3 default-size DATA frames).
///
/// Test sends a 60KB response, well under the 65535 default stream window so no
/// `WINDOW_UPDATE` is required, and expects all bytes plus the empty END_STREAM frame.
#[tokio::test]
async fn large_response_emits_all_frames_without_peer_window_update() {
    use trillium_http::Status;

    const BODY_LEN: usize = 60_000;
    let body: Vec<u8> = (0..BODY_LEN).map(|i| (i % 251) as u8).collect();
    let body_for_handler = body.clone();

    let (mut client_io, server_io) = duplex(256 * 1024);
    let (server_conn, server_task) =
        spawn_h2_server_with_handler(Compat::new(server_io), move |conn| {
            let body = body_for_handler.clone();
            async move { conn.with_status(Status::Ok).with_response_body(body) }
        });

    client_io.write_all(CLIENT_PREFACE).await.unwrap();
    write_empty_settings(&mut client_io).await;

    // GET / on stream 1, END_STREAM (no request body).
    let mut block = vec![0x82, 0x87];
    block.push(0x44);
    block.push(b"/".len() as u8);
    block.extend_from_slice(b"/");
    ...
    while !got_end_stream {
        ...
        .unwrap_or_else(|_| {
            panic!(
                "server stalled emitting response. body_so_far={} of {BODY_LEN}",
                response_body.len()
            )
        });
    ...
    assert!(got_response_headers);
    assert_eq!(response_body, body, "all body bytes delivered intact");
