...
impl DecoderDynamicTable {
    ...
    async fn run_reader_inner<T: AsyncRead + Unpin + Send>(
        &self,
        stream: &mut T,
    ) -> Result<(), H3Error> {
        loop {
            ...
            if first & ENC_INSTR_INSERT_WITH_NAME_REF != 0 {
                ...
                let name: HeaderName<'static> = if is_static {
                    ...
                    self.name_at_relative(name_index).ok_or_else(|| {
                ...
                self.insert(name, HeaderValue::from(value))
                    .inspect_err(|e| {
                        log::error!(
                            "QPACK encoder: insert (name ref) failed for [{name_for_log}: {}]: \
                             {e:?}",
                ...
                self.insert(name, HeaderValue::from(value))
                    .inspect_err(|e| {
                        log::error!(
                            "QPACK encoder: insert (literal) failed for [{name_for_log}: {}]: \
                             {e:?}",
                            String::from_utf8_lossy(&value_for_log)
                        );
                    })?;
