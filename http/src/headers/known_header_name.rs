...
macro_rules! known_headers {
    (
        ...
        ///
        /// Prefer a `KnownHeaderName` variant over a `&'static str` where possible — it lets
        /// trillium skip parsing the header entirely.
        ///
        /// Do not rely on the u8 representation being stable between releases.
        ...
        impl KnownHeaderName {
            #[cfg(test)]
            pub(crate) const VARIANTS: &[KnownHeaderName] = &[$(Self::$variant,)+];
            ...
            pub(crate) fn as_lower_str(&self) -> &'static str {
                match self {
                    $( Self::$variant => $lower, )+
                }
            }

            /// Look for a case sensitive exact byte match
            pub(crate) fn lowercase_byte_match(bytes: &[u8]) -> Option<Self> {
                let len = bytes.len();

                $( if len == $lower.len() && bytes == $lower.as_bytes() { return Some(Self::$variant); } )+
                None
            }
        }
...
known_headers! {
    ...
    ("Proxy-Authentication-Info", ProxyAuthenticationInfo, "proxy-authentication-info"),
