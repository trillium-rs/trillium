...
//!
//! Two halves:
//! - [`H2Driver::poll_flush_outbound`] drains `write_buf` to the transport and issues a
//!   subsequent flush when bytes were written.
//! - The `queue_*` helpers append encoded frames to `write_buf` and set
//!   `write_flush_pending` so the next `poll_flush_outbound` issues a flush.
//!
    ...
    T: AsyncRead + AsyncWrite + Unpin + Send,
{
    ...
    pub(super) fn queue_client_preface(&mut self) {
        self.write_buf.extend_from_slice(super::recv::CLIENT_PREFACE);
        self.write_flush_pending = true;
    }
