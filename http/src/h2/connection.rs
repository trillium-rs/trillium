...
//! - [`peer_settings_wait`]: the [`PeerSettings`] sync primitive that parks until the peer's
//!   first SETTINGS frame is applied.
//! - [`submit`]: send-staging API ([`submit_send`][H2Connection::submit_send],
//!   [`submit_upgrade`][H2Connection::submit_upgrade]) and client-side stream-open
//!   primitives ([`open_stream`][H2Connection::open_stream] /
...
#[cfg(feature = "unstable")]
#[allow(unused_imports)] // re-exports for h2.rs's `pub use connection::{ResponseHeaders, SubmitSend}`
