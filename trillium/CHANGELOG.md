...
- `Upgrade::push_path` and `Upgrade::pop_path`, mirroring `Conn::push_path`/`Conn::pop_path` so
  router implementations can rebuild nested path context when delegating `has_upgrade` and
  `upgrade`, with `Upgrade::path` returning the current frame like `Conn::path` does.
