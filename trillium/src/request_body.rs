...
pub struct RequestBody<'a>(trillium_http::ReceivedBody<'a, Box<dyn Transport>>);
...
impl<'a> From<trillium_http::ReceivedBody<'a, Box<dyn Transport>>> for RequestBody<'a> {
    fn from(received_body: trillium_http::ReceivedBody<'a, Box<dyn Transport>>) -> Self {
        Self(received_body)
    }
}
