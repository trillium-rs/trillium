...
pub trait QuicConfig<S: Server>: Send + 'static {
    ...
    /// calling [`bind`](Self::bind). The default returns `true`; only the `()` implementation
    /// overrides it.
...
/// A type-erased [`QuicTransportReceive`] stream — `Box<dyn QuicTransportReceive + Unpin + Send +
/// Sync>`.
