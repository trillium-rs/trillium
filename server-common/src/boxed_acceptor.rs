...
/// Object-safe sibling of [`Acceptor`], with the associated types erased to `Box<dyn Transport>` and
/// a boxed future, so acceptors of differing concrete types can be stored behind one trait object.
...
/// lets a single multi-listener server hold several — e.g. a plaintext listener (`()`) alongside one
/// or more TLS listeners. The erased [`Output`](Acceptor::Output) is `Box<dyn Transport>`.
///
