...
- `QuicTransportSend::stopped`, returning a future that resolves when the peer abandons a
  QUIC send stream. Defaults to `None`. The HTTP/3 connection handler passes it to
  trillium-http, which cannot otherwise observe QUIC stream resets, `STOP_SENDING`, or
  connection loss — those are invisible at the `AsyncRead`/`AsyncWrite` seam.
...
- Boxed acceptor errors have a cleaner debug format
- Peer IP addresses are emitted along with acceptor errors
