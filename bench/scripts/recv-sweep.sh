...
RESULTS=/home/ubuntu/trillium/bench/results/recv-sweep-$(date +%Y%m%d-%H%M%S)
mkdir -p "$RESULTS"
echo "results dir: $RESULTS"

# Body sizes to compare against /echo
...
# (label, path, body file) pairs — pure recv vs echo at matched sizes
TARGETS=(
    "recv64    /recv  /tmp/echo-64b"
    "recv16k   /recv  /tmp/echo-16k"
    "recv1m    /recv  /tmp/echo-1m"
    "echo64    /echo  /tmp/echo-64b"
    "echo16k   /echo  /tmp/echo-16k"
    "echo1m    /echo  /tmp/echo-1m"
)
...
run_one() {
    ...
    for spec in "${TARGETS[@]}"; do
        ...
        printf "  %-9s  %10s req/s  mean=%-8s cpu=%s%%\n" "$lbl" "$rps" "$mean" "$cpu"
    done
