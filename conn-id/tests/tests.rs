...
#[test]
fn test_defaults() {
    let app = (ConnId::new().with_seed(1), "ok");
    assert_ok!(get("/").on(&app), "ok", "x-request-id" => "ohOwAwv0lK");
    assert_ok!(get("/").on(&app), "ok", "x-request-id" => "DJA1GZigip");
    assert_ok!(
    ...
    let conn = get("/").on(&app);
    assert_eq!(conn.id(), "mv15faFHFq");
    assert_eq!(log_formatter::conn_id(&conn, true), "mv15faFHFq");
