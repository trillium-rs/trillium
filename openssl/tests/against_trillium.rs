...
use trillium_openssl::{OpenSslAcceptor, OpenSslConfig};
...
pub fn with_openssl_server<Fun, Fut>(tests: Fun)
where
    Fun: FnOnce(Url) -> Fut,
    Fut: Future<Output = Result<(), Box<dyn Error>>>,
{
    block_on(async move {
        ...
        let handle = config()
            .with_host("localhost")
            .with_port(port)
            .with_acceptor(OpenSslAcceptor::from_single_cert(&pem, &key))
            .spawn(handler());
        handle.info().await;
...
pub fn openssl_client() -> Client {
    Client::new(OpenSslConfig {
...
#[test(harness = with_openssl_server)]
async fn openssl_client_openssl_server(url: Url) -> Result<(), Box<dyn Error>> {
    let _ = openssl_client().get(url).await?.success()?;
    Ok(())
}

#[test(harness = with_openssl_server)]
async fn rustls_client_openssl_server(url: Url) -> Result<(), Box<dyn Error>> {
    let _ = rustls_client().get(url).await?.success()?;
    Ok(())
}

#[test(harness = with_openssl_server)]
async fn native_tls_client_openssl_server(url: Url) -> Result<(), Box<dyn Error>> {
    let _ = native_tls_client().get(url).await?.success()?;
    Ok(())
}

#[test(harness = with_rustls_server)]
async fn openssl_client_rustls_server(url: Url) -> Result<(), Box<dyn Error>> {
    let _ = openssl_client().get(url).await?.success()?;
    Ok(())
}

#[test(harness = with_native_tls_server)]
async fn openssl_client_native_tls_server(url: Url) -> Result<(), Box<dyn Error>> {
    let _ = openssl_client().get(url).await?.success()?;
    Ok(())
}
