...
mod tests {
    ...
    fn parse_from_to() {
        ...
        assert_eq!(parse("bytes=500-999"), Some(RangeSpec::FromTo(500, 999)));
    }
    ...
    #[test]
    fn resolve_from_to_clamped_at_end() {
        assert_eq!(resolve(RangeSpec::FromTo(0, 999_999), 1000), Some((0, 999)));
        assert_eq!(resolve(RangeSpec::FromTo(0, 99), 1000), Some((0, 99)));
    }

    #[test]
    fn resolve_from_to_unsatisfiable_when_start_at_or_past_end() {
        assert_eq!(resolve(RangeSpec::FromTo(1000, 2000), 1000), None);
        assert_eq!(resolve(RangeSpec::FromTo(2000, 3000), 1000), None);
    }

    #[test]
    fn resolve_open_ended() {
        assert_eq!(resolve(RangeSpec::From(500), 1000), Some((500, 999)));
        assert_eq!(resolve(RangeSpec::From(0), 1000), Some((0, 999)));
    ...
    #[test]
    fn resolve_empty_file_always_unsatisfiable() {
        assert_eq!(resolve(RangeSpec::FromTo(0, 0), 0), None);
        assert_eq!(resolve(RangeSpec::From(0), 0), None);
        assert_eq!(resolve(RangeSpec::Suffix(10), 0), None);
    }
    ...
    #[test]
    fn if_range_etag_strong_only() {
        // Weak etag in If-Range never matches per RFC 9110 14.1.2.
        // Both sides weak: still must not match.
    ...
    #[test]
    fn if_range_no_validators() {
        assert!(!if_range_matches("\"abc\"", None, None));
    }
}
